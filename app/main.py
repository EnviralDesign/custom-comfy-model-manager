...
def install_asyncio_exception_filter() -> None:
    """Suppress a noisy Windows asyncio transport race without hiding real errors."""
    ...
    def handler(loop: asyncio.AbstractEventLoop, context: dict) -> None:
        ...
        is_empty_write_assertion = (
            isinstance(exc, AssertionError)
            and str(exc) == "Data should not be empty"
            and "_SelectorSocketTransport._write_send" in handle
        )
        if is_empty_write_assertion:
            logger.debug("Suppressed asyncio empty-write transport assertion")
            return
